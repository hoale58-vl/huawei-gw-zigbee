    ...
    while (complete == 0)
    {
			...
			case XUSER_WAIT_END_ACK:
				...
				{	
					if(real_pack_counter > 1)//如果是在升级中途取消，则重启zigbee模块
					 {
					 	if(++ zbreset_timeout_counts <3){   //计数复位次数，强制跳出升级wifi固件     ，           hardy add
						zigbeeModeReset(); 
					 	}else{//zigbee ota fail,enter wifi ota mode
					 		otaContrlInfo.ota_type = 2;//wifi 固件升级开始
			...
			case XUSER_RESTART_APP:
				...
				parm[3] = 1;
				*/
				if(otaContrlInfo.ota_type == 1)// 只有在ZC xmode升级成功后才允许WIFI 固件升级，否则，ZC无法工作
				{//xmode升级一旦开始后，如果失败，必须等其升级成功后才允许继续WIFI 的升级
					otaContrlInfo.ota_type = 2;//wifi 固件升级开始
				...
				if(gwStateForRealTime.zigbeeZcBasicInfo.zcSoftVersion == 0)
				{
					...
					//userCheckSaveFlashAndReset(1);//证明zigbee初始化的时候未成功，所以在此需要重启设备 del
	...
	if(complete == 1){
		...
		if(zbota_complete_counts<1){
			...
			devOtaUpdateingFlag =1;
				//gwStateForRealTime.gwDevIsOta = 2;//用于闪灯标志
				//qcom_watchdog(0,0);
				//userXmodemTransport();
				//qcom_watchdog(1,15);
